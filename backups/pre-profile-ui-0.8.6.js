  ...
  class RecommenderApp {
        ...
        <aside class="drawer" role="dialog" aria-modal="true" aria-labelledby="bgmpr-title" aria-hidden="true">
          ...
          <main class="content" tabindex="-1">
            <div class="welcome" data-role="welcome">
              ...
              <h3>从你的收藏中发现下一部</h3>
              <p>组件会在本地分析评分、标签、制作人员和声优信息，排除所有已标记条目，并将推荐分页展示，每页 5 个。</p>
      ...
      return `
        <nav class="pagination" aria-label="推荐结果分页">
          ...
          <div class="page-status" aria-live="polite">
            ...
            <small>共 ${Number(total).toLocaleString("zh-CN")} 个结果 · 每页 ${RECOMMENDATION_PAGE_SIZE} 个</small>
          </div>
    ...
    renderRecommendations(recommendations, summary = {}, pagination = {}) {
      ...
      results.innerHTML = `
        ...
        <div class="recommendation-list">${recommendations.map((item, index) => this.recommendationCard(item, Number(pagination.startIndex || 0) + index)).join("")}</div>
        ${this.paginationMarkup(pagination)}
        <details class="method-note">
          ...
          <div class="method-body">
            <ol class="method-steps">
              ...
              <li><span class="step-number">3</span><div><strong>排序并保持多样</strong><p>排除所有已标记条目，融合相似度与质量分，再对完整结果统一去重与多样化；分页只负责展示。</p></div></li>
            </ol>
      ...
      return `<style>
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .launcher {
          ...
          z-index: 10; width: 160px; min-height: 56px; padding: 7px 11px 7px 8px;
        ...
        .pagination {
          margin-top: 16px; padding: 10px; border: 1px solid var(--border); border-radius: 14px; background: var(--surface-alt);
          display: grid; grid-template-columns: minmax(0, 1fr) auto minmax(0, 1fr); align-items: center; gap: 10px;
        }
        .page-button {
          min-width: 96px; min-height: 44px; padding: 0 12px; border: 1px solid var(--border); border-radius: 10px;
          background: var(--surface-raised); color: var(--text); display: inline-flex; align-items: center; justify-content: center; gap: 6px;
          font-size: 13px; font-weight: 750; transition: border-color 180ms ease-out, color 180ms ease-out, background 180ms ease-out;
        }
        .page-button:hover:not(:disabled) { border-color: var(--primary); background: var(--surface); color: var(--primary); }
        .page-button svg { width: 18px; height: 18px; }
        .page-previous { justify-self: start; }
        .page-next { justify-self: end; }
        .page-status { min-width: 132px; display: grid; justify-items: center; gap: 2px; color: var(--text-muted); }
        .page-status label { min-height: 28px; display: inline-flex; align-items: center; gap: 5px; font-size: 12px; font-weight: 700; }
        .page-select-shell { position: relative; width: 64px; height: 36px; display: inline-grid; }
        .page-status select {
          width: 100%; height: 36px; padding: 0 24px 0 10px; border: 1px solid var(--border); border-radius: 8px;
          appearance: none; -webkit-appearance: none; background: var(--surface-raised); color: var(--text); font-size: 13px; font-weight: 800;
          font-variant-numeric: tabular-nums; text-align: center; text-align-last: center; cursor: pointer;
        }
        .page-status select:hover { border-color: var(--primary); }
        .page-select-arrow { position: absolute; right: 6px; top: 50%; width: 16px; height: 16px; color: var(--text-muted); pointer-events: none; transform: translateY(-50%); display: grid; place-items: center; }
        .page-select-arrow svg { width: 16px; height: 16px; fill: currentColor; }
        .page-status small { font-size: 10px; line-height: 1.35; white-space: nowrap; }
        ...
        @media (max-width: 560px) {
          ...
          .pagination { gap: 7px; padding: 8px; }
          .page-button { min-width: 82px; padding-inline: 9px; }
          .page-status { min-width: 118px; }
        }
        @media (max-width: 390px) {
          ...
          .pagination { grid-template-columns: 44px minmax(0, 1fr) 44px; }
          .page-button { min-width: 44px; width: 44px; padding: 0; }
          .page-button span { position: absolute; width: 1px; height: 1px; padding: 0; margin: -1px; overflow: hidden; clip: rect(0, 0, 0, 0); white-space: nowrap; border: 0; }
...
(function bootstrapBangumiPersonalStats() {
  ...
  const TABS = Object.freeze({ overview: "回顾", staff: "创作构成", cast: "声优" });
  ...
  class StatsDrawer {
    ...
    detectTheme() { return /dark|night/i.test(`${document.documentElement.className} ${document.body?.className || ""}`) || matchMedia("(prefers-color-scheme: dark)").matches ? "dark" : "light"; }
    ...
    close() { this.state.open = false; this.render(); this.lastFocused?.focus?.(); }
    ...
    onKeyDown(event) { if (this.state.open && event.key === "Escape") { event.preventDefault(); this.close(); } }
    ...
    pager(kind, page, pages, total) {
      if (pages <= 1) return `<div class="list-meta">共 ${formatNumber(total)} 人</div>`;
      return `<div class="pager" aria-label="排名分页"><button type="button" data-action="page" data-page-kind="${kind}" data-page="${page - 1}" ${page <= 1 ? "disabled" : ""}>上一页</button><span>第 ${formatNumber(page)} / ${formatNumber(pages)} 页 · 共 ${formatNumber(total)} 人</span><button type="button" data-action="page" data-page-kind="${kind}" data-page="${page + 1}" ${page >= pages ? "disabled" : ""}>下一页</button></div>`;
    }
    listRows(rows, kind, pageKind) {
      if (!rows.length) return '<p class="empty">没有符合条件的结果。</p>';
      ...
      const pager = this.pager(pageKind, page, pages, rows.length);
      return `${pager}<ol class="rank-list" start="${offset + 1}">${shown.map((row, index) => {
        const ratingSample = row.ratedWorks < row.works ? ` · ${formatNumber(row.ratedWorks)} 部评分` : "";
        const detail = kind === "cast" ? `${formatNumber(row.works)} 部 · ${formatNumber(row.characterCount)} 角色 · 均分 ${formatRate(row.averageRate)}${ratingSample}` : `${formatNumber(row.works)} 部 · ${formatNumber(row.eps)} 话 · 均分 ${formatRate(row.averageRate)}${ratingSample}`;
        return `<li><span class="rank">${offset + index + 1}</span><a href="/person/${row.id}" target="_blank" rel="noopener">${escapeHtml(row.name)}</a><small>${detail}</small></li>`;
      }).join("")}</ol>${pages > 1 ? pager : ""}`;
    }
    overview(stats) {
      ...
      return `<div class="overview-grid"><section class="overview-metrics" aria-label="回顾概览"><div><span>总话数</span><strong>${formatNumber(stats.overview.watchedEpisodes)}</strong><small>${formatNumber(stats.overview.works)} 部看过动画的章节进度合计</small></div><div><span>今年已看</span><strong>${formatNumber(stats.overview.watchedThisYear)}</strong><small>${stats.overview.currentYear} 年标记为看过</small></div></section>${yearCard}${tagCard}${longestCard}</div>`;
    }
    staff(stats) {
      ...
      const source = stats.groups[active.id] || [];
      const ranking = Core.rankEntries(source, this.state.sort.staff, 0.1);
      const rows = this.filterRows(ranking.rows, this.state.search.staff);
      const chips = groups.map((group) => `<button type="button" data-action="staff-group" data-group="${group.id}" aria-pressed="${group.id === active.id}">${escapeHtml(group.label)}<small>${formatNumber(stats.groups[group.id]?.length || 0)}</small></button>`).join("");
      const summary = this.state.sort.staff === "average" ? `${formatNumber(ranking.eligibleCount)} 人入榜 · 至少 ${formatNumber(ranking.cutoffWorks)} 部` : `共 ${formatNumber(source.length)} 人`;
      return `<section class="ranking-panel"><div class="ranking-tools"><label><span class="sr-only">搜索${escapeHtml(active.label)}姓名</span>${ICONS.search}<input type="search" data-search="staff" value="${escapeHtml(this.state.search.staff)}" placeholder="搜索${escapeHtml(active.label)}姓名" autocomplete="off"></label>${this.sortControls("staff")}<p>${this.state.sort.staff === "average" ? `按个人均分排序；只有参与你看过作品数位于该职位前 10% 的人参与，当前门槛为 ${formatNumber(ranking.cutoffWorks)} 部。` : `每页 ${RANK_PAGE_SIZE} 人，按参与你看过的作品数排序。`}</p></div><div class="role-switch" aria-label="创作职位">${chips}</div><section class="table-card"><header><h3>${escapeHtml(active.label)}</h3><span>${this.state.search.staff ? `找到 ${formatNumber(rows.length)} / ${formatNumber(ranking.rows.length)} 人` : summary}</span></header>${this.listRows(rows, "staff", "staff")}</section></section>`;
    }
    cast(stats) {
      const source = stats.groups.cast;
      const ranking = Core.rankEntries(source, this.state.sort.cast, 0.1);
      const rows = this.filterRows(ranking.rows, this.state.search.cast);
      const summary = this.state.sort.cast === "average" ? `${formatNumber(ranking.eligibleCount)} 人入榜 · 至少 ${formatNumber(ranking.cutoffWorks)} 部` : `共 ${formatNumber(source.length)} 人`;
      return `<section class="ranking-panel"><div class="ranking-tools"><label><span class="sr-only">搜索声优姓名</span>${ICONS.search}<input type="search" data-search="cast" value="${escapeHtml(this.state.search.cast)}" placeholder="搜索声优姓名" autocomplete="off"></label>${this.sortControls("cast")}<p>${this.state.sort.cast === "average" ? `按个人均分排序；只有参演你看过作品数位于所有声优前 10% 的人参与，当前门槛为 ${formatNumber(ranking.cutoffWorks)} 部。` : "同一声优在同一作品中只计 1 部；角色数单独累计。"}</p></div><section class="cast-card"><header><div><h3>声优构成</h3></div><span>${this.state.search.cast ? `找到 ${formatNumber(rows.length)} / ${formatNumber(ranking.rows.length)} 人` : summary}</span></header>${this.listRows(rows, "cast", "cast")}</section></section>`;
    }
    ...
    render() {
      ...
      const progressCount = progress.countText ?? (progress.total ? `${formatNumber(progress.current)} / ${formatNumber(progress.total)}` : "");
      const percent = progress.total ? Math.min(100, Math.round(progress.current / progress.total * 100)) : 0;
      const tabs = Object.entries(TABS).map(([id, label]) => `<button type="button" role="tab" aria-selected="${this.state.activeTab === id}" data-action="tab" data-tab="${id}">${label}</button>`).join("");
      this.shadow.innerHTML = `${this.styles()}<button class="launcher" data-action="open" aria-label="打开动画回顾统计" aria-haspopup="dialog"><span class="launcher-mark">${ICONS.chart}</span><span class="launcher-copy"><small>WATCHED</small><strong>动画回顾</strong></span><span class="launcher-arrow">${ICONS.launchArrow}</span></button><div class="scrim" ${this.state.open ? "" : "hidden"} data-action="close"></div><aside class="drawer ${this.state.open ? "open" : ""}" role="dialog" aria-modal="true" aria-labelledby="bgmstats-title" ${this.state.open ? "" : 'aria-hidden="true"'}><header class="drawer-head"><div><p>FOR ${DEFAULT_USER.toUpperCase()} · WATCHED ONLY</p><h2 id="bgmstats-title">动画回顾</h2><span>只统计你标记为“看过”的动画</span></div><button class="close" data-action="close" aria-label="关闭动画统计">${ICONS.close}</button></header><div class="control-row"><div class="sample-note"><i>${ICONS.users}</i><span><strong>${formatNumber(stats.overview.works)} 部看过动画</strong><small>不混入想看、在看、搁置或抛弃</small></span></div><div><button class="refresh" data-action="sync" ${this.state.busy ? "disabled" : ""}>${ICONS.refresh}<span>同步</span></button><button class="primary refresh" data-action="all" ${this.state.busy || !stats.overview.works ? "disabled" : ""}>补全数据</button></div></div><div class="progress" aria-live="polite"><span data-role="progress-label">${escapeHtml(progress.label)}</span><strong data-role="progress-count">${progressCount}</strong><i><b data-role="progress-bar" style="width:${percent}%"></b></i>${this.state.busy ? '<button data-action="cancel">暂停</button>' : ""}</div><div class="kpis"><div><span>个人均分</span><strong>${formatRate(stats.overview.averageRate)}</strong><small>${formatNumber(stats.overview.ratedWorks)} 部已评分</small></div><div><span>总话数</span><strong>${formatNumber(stats.overview.knownEpisodes)}</strong><small>每部约 ${formatRate(stats.overview.averageEpisodes)} 话</small></div><div><span>已看话数</span><strong>${formatNumber(stats.overview.watchedEpisodes)}</strong><small>来自章节进度</small></div></div><nav role="tablist">${tabs}</nav><main class="content">${this.content(stats)}</main><footer><span>${this.state.lastSync ? `同步于 ${new Date(this.state.lastSync).toLocaleString("zh-CN", { hour12: false })}` : "数据仅保存在当前浏览器"}</span><em>v${APP_VERSION}</em></footer></aside>`;
    }
    styles() { return `<style>
      .content .expandable-card>.year-list{grid-template-rows:repeat(var(--year-rows),auto)!important;grid-auto-flow:column!important}@media(max-width:600px){.content .expandable-card>.year-list{grid-template-rows:none!important;grid-auto-flow:row!important}}
      aside.drawer{grid-template-rows:auto auto auto auto minmax(0,1fr) auto}aside.drawer>.kpis{display:none}.content .overview-metrics{grid-column:1/-1;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.content .overview-metrics>div{min-height:64px;padding:10px 13px;border:1px solid var(--line);border-radius:11px;background:var(--surface);display:grid;grid-template-columns:auto 1fr;grid-template-rows:auto auto;column-gap:10px;align-items:center}.content .overview-metrics span{grid-row:1;color:var(--muted);font-size:10px;font-weight:750}.content .overview-metrics strong{grid-row:1/3;font-size:22px;font-variant-numeric:tabular-nums}.content .overview-metrics small{grid-column:1/-1;color:var(--muted);font-size:10px}.content .overview-grid>.expandable-card{grid-column:1/-1}.content .expandable-card>.year-list{grid-template-columns:repeat(2,minmax(0,1fr));column-gap:22px}.content .expand-control{width:100%;min-height:42px;padding:0 13px;border:0;border-top:1px solid var(--line);background:color-mix(in srgb,var(--soft) 72%,var(--surface));color:var(--primary);display:flex;align-items:center;justify-content:center;gap:7px;font-size:11px;font-weight:750}.content .expand-control:hover{background:color-mix(in srgb,var(--primary) 8%,var(--surface))}.content .expand-control svg{width:15px;height:15px;fill:none;stroke:currentColor;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round;transition:transform 160ms ease}.content .expand-control[aria-expanded="true"] svg{transform:rotate(180deg)}.content .longest-list a{overflow:visible;white-space:normal;text-overflow:clip;line-height:1.45}.content .longest-list li{padding:6px 0;align-items:flex-start}.content .ranking-panel{display:grid;gap:10px}.content .ranking-tools{position:sticky;top:-18px;z-index:3;padding:10px;border:1px solid var(--line);border-radius:12px;background:color-mix(in srgb,var(--soft) 92%,var(--surface));box-shadow:0 8px 18px color-mix(in srgb,var(--ink) 7%,transparent)}.content .ranking-tools label{min-height:44px;padding:0 12px;border:1px solid var(--line);border-radius:10px;background:var(--surface);display:flex;align-items:center;gap:9px}.content .ranking-tools label:focus-within{border-color:var(--primary);box-shadow:0 0 0 3px color-mix(in srgb,var(--primary) 12%,transparent)}.content .ranking-tools label svg{width:18px;height:18px;flex:0 0 auto;fill:none;stroke:var(--muted);stroke-width:1.8}.content .ranking-tools input{width:100%;min-width:0;border:0;outline:0;background:transparent;color:var(--ink);font-size:13px}.content .ranking-tools p{margin:7px 2px 0;color:var(--muted);font-size:10px;line-height:1.5}.content .sort-row{margin-top:8px;display:flex;align-items:center;gap:8px}.content .sort-row>span{color:var(--muted);font-size:10px;font-weight:750}.content .sort-row>small{margin-left:auto;color:var(--muted);font-size:10px}.content .sort-switch{padding:2px;border:1px solid var(--line);border-radius:9px;background:var(--surface);display:flex}.content .sort-switch button{min-width:64px;min-height:32px;padding:0 10px;border:0;border-radius:7px;background:transparent;color:var(--muted);font-size:11px;font-weight:750}.content .sort-switch button[aria-pressed="true"]{background:var(--primary);color:#fff;box-shadow:0 2px 6px color-mix(in srgb,var(--primary) 24%,transparent)}.content .role-switch{display:flex;flex-wrap:wrap;gap:6px}.content .role-switch button{min-height:36px;padding:0 10px;border:1px solid var(--line);border-radius:9px;background:var(--surface);color:var(--muted);font-size:11px;font-weight:750}.content .role-switch button[aria-pressed="true"]{border-color:var(--primary);background:color-mix(in srgb,var(--primary) 10%,var(--surface));color:var(--primary)}.content .role-switch small{margin-left:5px;font-size:9px;font-variant-numeric:tabular-nums}.content .pager{min-height:48px;padding:7px 12px;border-bottom:1px solid var(--line);display:grid;grid-template-columns:auto 1fr auto;align-items:center;gap:8px}.content .rank-list+.pager{border-top:1px solid var(--line);border-bottom:0}.content .pager button{min-height:34px;padding:0 10px;border:1px solid var(--line);border-radius:8px;background:var(--surface);color:var(--ink);font-size:11px;font-weight:700}.content .pager button:hover:not(:disabled){border-color:var(--primary);color:var(--primary)}.content .pager span,.content .list-meta{color:var(--muted);font-size:10px;text-align:center;font-variant-numeric:tabular-nums}.content .list-meta{padding:10px 13px;border-bottom:1px solid var(--line)}.content .rank-list a,.content .rank-list small{overflow:visible;white-space:normal;text-overflow:clip;line-height:1.4}.content .rank-list li{padding:6px 0;min-height:46px}.sr-only{position:absolute!important;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}@media(max-width:600px){.content .overview-metrics{grid-template-columns:1fr}.content .expandable-card>.year-list{grid-template-columns:1fr}.content .ranking-tools{top:-18px}.content .sort-row{align-items:flex-start;flex-wrap:wrap}.content .sort-row>small{width:100%;margin-left:0}.content .role-switch button{flex:1 1 auto}.content .pager{grid-template-columns:auto 1fr auto}.content .pager button{padding:0 8px}}
      :host{all:initial;--ink:#33282c;--muted:#796d72;--surface:#fffdfd;--soft:#f8f3f4;--line:#e7dcdf;--primary:#ad3f5e;--primary-deep:#8f304b;--accent:#177c70;--scrim:rgba(30,19,23,.48);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI","Microsoft YaHei",sans-serif}:host([data-theme="dark"]){--ink:#f8f0f2;--muted:#c8b9bf;--surface:#241c1f;--soft:#2d2327;--line:#4a3b40;--primary:#d96887;--primary-deep:#e47a98;--accent:#73cbbd;--scrim:rgba(0,0,0,.66)}.launcher,.drawer{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI","Microsoft YaHei",sans-serif}.refresh{display:inline-flex;align-items:center;justify-content:center;gap:4px}.control-row .refresh svg{margin:0;vertical-align:middle}.rank-list li{content-visibility:auto;contain-intrinsic-size:auto 42px}
      button.launcher{width:160px;padding:7px 11px 7px 8px;display:inline-flex;gap:10px}button.launcher>.launcher-mark{width:40px;height:40px;flex:0 0 auto;border-radius:12px;display:grid;place-items:center;background:var(--primary);color:#fff}.launcher-mark svg{width:23px;height:23px;fill:none;stroke:currentColor;stroke-width:1.75;stroke-linecap:round;stroke-linejoin:round}button.launcher>.launcher-copy{width:auto;height:auto;min-width:62px;border-radius:0;background:transparent;color:inherit;display:grid;gap:2px;place-items:initial;line-height:1}.launcher-copy small{color:var(--primary);font-size:9px;font-weight:850;letter-spacing:.15em}.launcher-copy strong{white-space:nowrap;font-size:14px;font-weight:750;letter-spacing:.01em}button.launcher>.launcher-arrow{width:16px;height:20px;flex:0 0 auto;border-radius:0;background:transparent;color:var(--muted);display:grid;place-items:center;transition:color 180ms ease-out,transform 180ms ease-out}.launcher-arrow svg{width:16px;height:16px;fill:none;stroke:currentColor;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round}button.launcher:hover>.launcher-arrow{color:var(--primary);transform:translateX(2px)}@media(max-width:390px){button.launcher{width:54px}button.launcher>.launcher-copy,button.launcher>.launcher-arrow{display:none}button.launcher>.launcher-mark{width:40px;height:40px}}
      :host{all:initial}#bgmstats-host{--ink:#33282c;--muted:#796d72;--surface:#fffdfd;--soft:#f8f3f4;--line:#e7dcdf;--primary:#ad3f5e;--primary-deep:#8f304b;--accent:#177c70;--scrim:rgba(30,19,23,.48);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI","Microsoft YaHei",sans-serif}#bgmstats-host[data-theme="dark"]{--ink:#f8f0f2;--muted:#c8b9bf;--surface:#241c1f;--soft:#2d2327;--line:#4a3b40;--primary:#d96887;--primary-deep:#e47a98;--accent:#73cbbd;--scrim:rgba(0,0,0,.66)}*,*::before,*::after{box-sizing:border-box}button,a{font:inherit}button{cursor:pointer}button:disabled{cursor:not-allowed;opacity:.48}button:focus-visible,a:focus-visible{outline:3px solid color-mix(in srgb,var(--primary) 50%,transparent);outline-offset:2px}.launcher{position:fixed;z-index:9998;right:max(20px,env(safe-area-inset-right));bottom:max(146px,calc(env(safe-area-inset-bottom) + 90px));min-height:56px;padding:7px 13px 7px 8px;border:1px solid color-mix(in srgb,var(--primary) 22%,var(--line));border-radius:17px;background:color-mix(in srgb,var(--surface) 94%,transparent);color:var(--ink);box-shadow:0 2px 8px rgba(63,35,45,.12),0 14px 34px rgba(84,42,54,.16);display:flex;align-items:center;gap:9px;text-align:left;backdrop-filter:blur(14px);transition:transform 160ms ease,box-shadow 180ms ease,border-color 180ms ease}.launcher:hover{transform:translateY(-2px);border-color:var(--primary);box-shadow:0 5px 14px rgba(63,35,45,.16),0 18px 40px rgba(84,42,54,.22)}.launcher:active{transform:scale(.98)}.launcher>span{width:40px;height:40px;border-radius:12px;display:grid;place-items:center;background:var(--primary);color:#fff}.launcher svg{width:22px;height:22px;fill:none;stroke:currentColor;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round}.launcher i{display:grid;gap:2px;font-style:normal}.launcher small{color:var(--primary);font-size:9px;font-weight:850;letter-spacing:.14em}.launcher strong{font-size:14px;line-height:1}.scrim{position:fixed;z-index:9999;inset:0;background:var(--scrim)}.scrim[hidden]{display:none}.drawer{position:fixed;z-index:10000;top:0;right:0;width:min(680px,100vw);height:100dvh;overflow:hidden;display:grid;grid-template-rows:auto auto auto auto auto minmax(0,1fr) auto;background:var(--surface);color:var(--ink);box-shadow:-20px 0 60px rgba(35,19,26,.28);transform:translateX(102%);transition:transform 240ms ease-out}.drawer.open{transform:translateX(0)}.drawer[aria-hidden="true"]{pointer-events:none}.drawer-head{min-height:105px;padding:23px 25px 18px;display:flex;justify-content:space-between;gap:18px;border-bottom:1px solid var(--line);background:linear-gradient(135deg,color-mix(in srgb,var(--surface) 88%,#fff) 0%,color-mix(in srgb,var(--primary) 8%,var(--surface)) 100%)}.drawer-head p{margin:0 0 5px;color:var(--primary);font-size:10px;font-weight:850;letter-spacing:.14em}h2{margin:0;font-size:26px;letter-spacing:-.035em}.drawer-head span{display:block;margin-top:5px;color:var(--muted);font-size:12px}.close{width:44px;height:44px;flex:0 0 auto;border:1px solid var(--line);border-radius:12px;background:var(--surface);color:var(--muted);display:grid;place-items:center}.close:hover{color:var(--primary);border-color:var(--primary)}.close svg{width:19px;height:19px;fill:none;stroke:currentColor;stroke-width:2}.control-row{min-height:70px;padding:12px 25px;display:flex;justify-content:space-between;gap:12px;align-items:center;border-bottom:1px solid var(--line);background:var(--soft)}.sample-note{display:flex;align-items:center;gap:9px;min-width:0}.sample-note i{width:35px;height:35px;flex:0 0 auto;border-radius:10px;display:grid;place-items:center;background:color-mix(in srgb,var(--primary) 10%,transparent);color:var(--primary)}.sample-note svg{width:18px;height:18px;fill:none;stroke:currentColor;stroke-width:1.8}.sample-note span{min-width:0;display:grid;gap:2px}.sample-note strong{font-size:12px}.sample-note small{overflow:hidden;color:var(--muted);font-size:10px;text-overflow:ellipsis;white-space:nowrap}.control-row>div:last-child,.data-actions{display:flex;gap:7px;flex-wrap:wrap;justify-content:flex-end}.refresh,.data-actions button{min-height:36px;padding:0 10px;border:1px solid var(--line);border-radius:9px;background:var(--surface);color:var(--ink);font-size:12px;font-weight:750}.refresh:hover,.data-actions button:hover{border-color:var(--primary);color:var(--primary)}.refresh svg{width:15px;height:15px;margin:-2px 3px -3px 0;fill:none;stroke:currentColor;stroke-width:2;vertical-align:middle}.primary{border-color:var(--primary)!important;background:var(--primary)!important;color:#fff!important}.primary:hover{background:var(--primary-deep)!important}.progress{min-height:43px;padding:9px 25px;display:grid;grid-template-columns:minmax(0,1fr) auto;gap:4px 12px;align-items:center;color:var(--muted);font-size:11px}.progress strong{color:var(--primary);font-variant-numeric:tabular-nums}.progress i{grid-column:1/-1;height:3px;overflow:hidden;border-radius:99px;background:var(--soft)}.progress i b{display:block;height:100%;border-radius:inherit;background:var(--primary);transition:width 180ms ease}.progress button{grid-column:2;grid-row:1;min-height:24px;padding:0 6px;border:0;background:transparent;color:var(--primary);font-size:11px}.kpis{padding:0 25px 17px;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:9px}.kpis>div{min-height:89px;padding:12px;border:1px solid var(--line);border-radius:11px;background:var(--soft);display:grid;align-content:start;gap:3px}.kpis span{color:var(--muted);font-size:10px;font-weight:750}.kpis strong{font-size:23px;font-variant-numeric:tabular-nums}.kpis small{color:var(--muted);font-size:10px}nav{display:flex;gap:1px;padding:0 25px;border-bottom:1px solid var(--line)}nav button{position:relative;min-height:42px;padding:0 12px;border:0;background:transparent;color:var(--muted);font-size:12px;font-weight:800}nav button[aria-selected="true"]{color:var(--primary)}nav button[aria-selected="true"]::after{position:absolute;right:9px;bottom:-1px;left:9px;height:2px;border-radius:99px 99px 0 0;background:var(--primary);content:""}.content{min-height:0;overflow:auto;padding:18px 25px 26px;background:var(--soft)}.overview-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.wide-card,.table-card,.cast-card,.data-card{overflow:hidden;border:1px solid var(--line);border-radius:12px;background:var(--surface)}.wide-card header,.table-card header,.cast-card header{padding:13px;display:flex;justify-content:space-between;align-items:baseline;gap:10px;border-bottom:1px solid var(--line)}h3,h4{margin:0;font-size:13px}header span{color:var(--muted);font-size:10px}.score-card{grid-column:1/-1}.score-bars{height:138px;padding:17px 15px 11px;display:grid;grid-template-columns:repeat(10,minmax(0,1fr));align-items:end;gap:7px}.score-bars div{height:100%;display:grid;grid-template-rows:auto 1fr auto;justify-items:center;gap:5px;color:var(--muted);font-size:10px}.score-bars i{position:relative;width:100%;height:100%;border-radius:5px 5px 2px 2px;background:color-mix(in srgb,var(--primary) 8%,var(--soft));display:flex;align-items:end;overflow:hidden}.score-bars b{width:100%;min-height:2px;border-radius:inherit;background:linear-gradient(180deg,var(--primary),var(--primary-deep))}.score-bars strong{min-height:11px;font-size:9px;font-variant-numeric:tabular-nums}.year-list{padding:12px;display:grid;gap:7px}.year-list span{display:grid;grid-template-columns:38px minmax(0,1fr) 28px;align-items:center;gap:7px;color:var(--muted);font-size:10px}.year-list b{color:var(--ink);font-weight:750}.year-list i{position:relative;height:5px;overflow:hidden;border-radius:8px;background:var(--soft)}.year-list i::after{display:block;width:var(--share);height:100%;border-radius:inherit;background:var(--accent);content:""}.year-list small{text-align:right;font-variant-numeric:tabular-nums}.tag-cloud{padding:12px;display:flex;flex-wrap:wrap;gap:6px}.tag-cloud span{padding:4px 7px;border:1px solid var(--line);border-radius:999px;background:var(--soft);color:var(--ink);font-size:11px}.tag-cloud b{margin-left:4px;color:var(--primary);font-size:9px}.longest-list{margin:0;padding:4px 13px 10px;list-style:none}.longest-list li{min-height:31px;display:flex;justify-content:space-between;align-items:center;gap:12px;border-bottom:1px solid color-mix(in srgb,var(--line) 58%,transparent)}.longest-list li:last-child{border:0}.longest-list a{overflow:hidden;color:var(--ink);font-size:11px;text-decoration:none;text-overflow:ellipsis;white-space:nowrap}.longest-list a:hover{color:var(--primary);text-decoration:underline}.longest-list strong{flex:0 0 auto;color:var(--primary);font-size:10px;font-variant-numeric:tabular-nums}.staff-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.table-card header h4{font-size:12px}.rank-list{margin:0;padding:4px 13px 8px;list-style:none}.rank-list li{min-height:42px;display:grid;grid-template-columns:20px minmax(0,1fr);column-gap:8px;align-content:center;border-bottom:1px solid color-mix(in srgb,var(--line) 58%,transparent)}.rank-list li:last-child{border:0}.rank{grid-row:1/3;width:19px;height:19px;border-radius:6px;background:var(--soft);color:var(--muted);font-size:9px;font-weight:800;display:grid;place-items:center}.rank-list li:nth-child(-n+3) .rank{background:color-mix(in srgb,var(--primary) 12%,var(--soft));color:var(--primary)}.rank-list a{overflow:hidden;color:var(--ink);font-size:12px;font-weight:700;text-decoration:none;text-overflow:ellipsis;white-space:nowrap}.rank-list a:hover{color:var(--primary);text-decoration:underline}.rank-list small{overflow:hidden;color:var(--muted);font-size:10px;text-overflow:ellipsis;white-space:nowrap}.cast-card header{align-items:center}.cast-card header p{margin:5px 0 0;color:var(--muted);font-size:11px;line-height:1.5}.cast-card header>strong{color:var(--primary);font-size:18px;white-space:nowrap;font-variant-numeric:tabular-nums}.cast-card header strong small{color:var(--muted);font-size:10px;font-weight:400}.data-card{padding:17px}.data-intro{display:flex;gap:11px;align-items:flex-start}.data-intro>i{width:38px;height:38px;flex:0 0 auto;border-radius:11px;display:grid;place-items:center;background:color-mix(in srgb,var(--primary) 10%,transparent);color:var(--primary)}.data-intro svg{width:19px;height:19px;fill:none;stroke:currentColor;stroke-width:1.8}.data-intro p{margin:5px 0 0;color:var(--muted);font-size:12px;line-height:1.6}.coverage{margin:17px 0;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}.coverage>div{padding:11px;border:1px solid var(--line);border-radius:10px;background:var(--soft);display:grid;gap:5px}.coverage span{color:var(--muted);font-size:10px}.coverage strong{font-size:15px;font-variant-numeric:tabular-nums}.coverage small{color:var(--muted);font-size:10px;font-weight:400}.empty{margin:0;padding:18px 13px;color:var(--muted);font-size:12px;text-align:center}footer{min-height:54px;padding:10px 25px max(10px,env(safe-area-inset-bottom));border-top:1px solid var(--line);background:var(--surface);color:var(--muted);font-size:10px;display:flex;justify-content:space-between;align-items:center;gap:12px}footer em{color:var(--primary);font-style:normal;font-variant-numeric:tabular-nums}@media(max-width:600px){.drawer-head,.control-row,.progress,.content,footer{padding-left:16px;padding-right:16px}.kpis,nav{padding-left:16px;padding-right:16px}.drawer-head{min-height:96px;padding-top:18px}h2{font-size:23px}.control-row{align-items:flex-start;flex-direction:column}.control-row>div:last-child{width:100%;justify-content:flex-end}.overview-grid,.staff-grid{grid-template-columns:1fr}.score-card{grid-column:auto}.coverage{grid-template-columns:1fr}.launcher{right:12px;bottom:max(136px,calc(env(safe-area-inset-bottom) + 80px)}}@media(max-width:390px){.launcher{width:54px;height:54px;min-height:54px;padding:7px;border-radius:17px}.launcher>span{width:40px;height:40px}.launcher i{display:none}.kpis{grid-template-columns:repeat(3,minmax(0,1fr));gap:6px}.kpis>div{min-height:82px;padding:9px}.kpis strong{font-size:18px}.kpis small{font-size:9px}nav button{padding:0 7px;font-size:11px}}@media(prefers-reduced-motion:reduce){*{transition:none!important;animation:none!important;scroll-behavior:auto!important}}
      .launcher{width:160px;padding:7px 11px 7px 8px;display:inline-flex;gap:10px}.launcher>.launcher-mark{width:40px;height:40px;flex:0 0 auto;border-radius:12px;display:grid;place-items:center;background:var(--primary);color:#fff}.launcher-mark svg{width:23px;height:23px;fill:none;stroke:currentColor;stroke-width:1.75;stroke-linecap:round;stroke-linejoin:round}.launcher>.launcher-copy{width:auto;height:auto;min-width:62px;border-radius:0;background:transparent;color:inherit;display:grid;gap:2px;place-items:initial;line-height:1}.launcher-copy small{color:var(--primary);font-size:9px;font-weight:850;letter-spacing:.15em}.launcher-copy strong{white-space:nowrap;font-size:14px;font-weight:750;letter-spacing:.01em}.launcher>.launcher-arrow{width:16px;height:20px;flex:0 0 auto;border-radius:0;background:transparent;color:var(--muted);display:grid;place-items:center;transition:color 180ms ease-out,transform 180ms ease-out}.launcher-arrow svg{width:16px;height:16px;fill:none;stroke:currentColor;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round}.launcher:hover .launcher-arrow{color:var(--primary);transform:translateX(2px)}@media(max-width:390px){.launcher{width:54px}.launcher>.launcher-copy,.launcher>.launcher-arrow{display:none}.launcher>.launcher-mark{width:40px;height:40px}}
    </style>`; }
  }
