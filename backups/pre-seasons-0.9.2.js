...
// @version      0.9.2
// @description  个人主页的动画回顾与个性推荐：完整年代柱图、标签词云、话数分布与人物排行。
// @author       wylt
// @match        https://bgm.tv/*
// @match        http://bgm.tv/*
...
(function attachStatsViz(global) {
  ...
  const api = { yearSeries, axis, fontSize, overlaps, packCloud, episodeDistribution, pieSlices };
...
(function bootstrapBangumiPersonalStats() {
  ...
  class StatsDrawer {
    ...
    years(rows) {
      ...
      return `<section class="year-chart" aria-label="看过动画的全部首播年份分布">
        ...
        <div class="year-columns" style="--columns:${series.length}">${series.map(row => {
          ...
          const five = boundary || (row.year % 5 === 0 && row.year - first >= 3 && last - row.year >= 3);
          const ten = boundary || (row.year % 10 === 0 && row.year - first >= 5 && last - row.year >= 5);
    ...
    tags(rows) {
      ...
      const caption = '个人标签';
      return `<section aria-label="全部个人标签词云"><div class="viz-heading"><span class="viz-caption" data-default="${caption}" aria-live="polite">${caption}</span></div><div class="tag-cloud">${rows.map((row, index) => `<button class="cloud-word" data-viz-label="${escapeHtml(row.name)} · ${row.count} 部" title="${escapeHtml(row.name)} · ${row.count} 部" aria-label="${escapeHtml(row.name)}，${row.count} 部" data-size="${Viz.fontSize(row.count, min, max)}" style="font-size:${Viz.fontSize(row.count, min, max)}px;--word-color:var(${index % 5 === 0 ? '--ink' : index % 5 === 3 ? '--muted' : '--link'})">${escapeHtml(row.name)}</button>`).join('')}</div></section>`;
    }
    works(rows) {
      const distribution = Viz.episodeDistribution(rows);
      if (!distribution.total) return '<p class="empty">暂无话数资料。</p>';
      const slices = Viz.pieSlices(distribution.groups);
      const unknown = this.state.collections.length - distribution.total;
      return `<section aria-label="看过动画的话数分布"><div class="viz-heading" title="按作品部数计算占比；单列占比至少 3% 的前五种话数，其余合并为其他。">话数分布</div><div class="episode-distribution"><svg class="episode-pie" viewBox="0 0 240 240" aria-hidden="true">${slices.map((slice, index) => `<path d="${slice.path}" style="fill:var(--slice-${index})"><title>${slice.label}：${slice.count} 部（${(slice.share * 100).toFixed(1)}%）</title></path>${slice.share >= 0.08 ? `<text x="${slice.labelX}" y="${slice.labelY}" text-anchor="middle" dominant-baseline="middle">${Math.round(slice.share * 100)}%</text>` : ''}`).join('')}</svg><ul class="episode-legend">${slices.map((slice, index) => `<li${slice.members ? ` title="${slice.members.map(row => row.eps + ' 话：' + row.count + ' 部').join('；')}"` : ''}><i style="background:var(--slice-${index})" aria-hidden="true"></i><span>${slice.label}</span><b>${formatNumber(slice.count)} 部</b><small>${(slice.share * 100).toFixed(1)}%</small></li>`).join('')}</ul></div>${unknown > 0 ? `<p class="distribution-note">${unknown} 部暂无话数资料，未计入比例</p>` : ''}</section>`;
    }
    scheduleCloud() {
    ...
    styles() { return `<style>${globalThis.BangumiProfileUI.css}
      ...
      .episode-distribution{--slice-0:#e88b98;--slice-1:#f3b1b9;--slice-2:#d5a0aa;--slice-3:#89b8c5;--slice-4:#b9a9c9;--slice-5:#c8c8c8;display:grid;grid-template-columns:minmax(200px,280px) minmax(0,1fr);align-items:center;gap:36px;max-width:650px;margin:0 auto}.episode-pie{width:100%;height:auto}.episode-pie path{stroke:var(--surface);stroke-width:2}.episode-pie text{fill:#38292c;font:12px Arial,sans-serif;pointer-events:none}.episode-legend{list-style:none;margin:0;padding:0;display:grid;gap:12px}.episode-legend li{display:grid;grid-template-columns:10px minmax(60px,1fr) auto 46px;align-items:center;gap:10px;padding-bottom:8px;border-bottom:1px solid var(--line)}.episode-legend i{width:9px;height:9px;border-radius:50%}.episode-legend b{font-weight:400;color:var(--ink);font-variant-numeric:tabular-nums}.episode-legend small{text-align:right;color:var(--muted);font-variant-numeric:tabular-nums}.distribution-note{text-align:center;color:var(--muted);font-size:11px;margin-top:16px}
      ...
      @container(max-width:500px){.people-list{grid-auto-flow:row;grid-template-rows:none;grid-template-columns:1fr;gap:20px}.sort-row{flex-wrap:wrap}.year-plot{height:190px;margin-left:30px}.year-columns{gap:1px}.year-column[data-label-five="true"] .column-year{display:none}.year-column[data-label-ten="true"] .column-year{display:block}.episode-distribution{grid-template-columns:1fr;gap:22px}.episode-pie{max-width:250px;justify-self:center}.episode-legend{width:100%}}
    </style>`; }
  }
